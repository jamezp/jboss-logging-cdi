...
public class LoggingExtension implements Extension {
    ...
    <T> void processAnnotatedType(@Observes ProcessAnnotatedType<T> pat) {
        ...
        LOGGER.infof("processAnnotatedType: %s", annotatedType);
    }

    void findProducer(@Observes ProcessProducerMethod<LoggingProducer, Object> producer) {
        LOGGER.infof("findProducer: %s", producer);
    }

    <T> void processInjectionTarget(@Observes ProcessInjectionTarget<T> pit) {
        LOGGER.infof("processInjectionTarget: %s", pit);
    }
